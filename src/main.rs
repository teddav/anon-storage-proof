...
fn main() {
    ...
    let block_storage_input = EthBlockStorageInput {
        ...
        block_number: block.number.unwrap().try_into().unwrap(),
        storage: storage_input,
        block_header: get_block_rlp(&block),
    };
    let circuit = EthBlockStorageCircuit::<Fr>::new(block_storage_input, Chain::AnvilHardhat);
    // println!("{circuit:?}");
